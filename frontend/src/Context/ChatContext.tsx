...
import { LocalStorage, requestHandler } from "../utils/helper";
...
const ChatProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const getChats = async () => {
    requestHandler(
      ...
      (e) => toast({description:e})
    );
  };

  const getMessages = async () => {
    // Check if a chat is selected, if not, show an alert
    if (!currentChat.current?._id) return toast({description:"No chat is selected"})

    // Check if socket is available, if not, show an alert
    if (!socket) return toast({description:"Socket not available"})
